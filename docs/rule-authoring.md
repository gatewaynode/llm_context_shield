...
|----------------|----------|---------|-------------------------------------------|---------------------------------------|
| `category`     | yes      | string  | `prompt_injection`, `jailbreak`, `data_exfiltration`, `hidden_content`, `delimiter_manipulation`, `instruction_override`, `refusal_suppression`, `response_steering`, `secret_probing` | Finding category, severity filtering |
